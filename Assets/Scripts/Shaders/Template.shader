Shader "Converted/Template"
{
    ...
    SubShader
    {
        Pass
        {
            ...
            {
                float q = max(p.x, -0.1);
                float s = sin(p.x * 7. + time * 70.) * 0.08;
                p.y += s;
                p.y *= 1.1;
                float4 c;
                if (p.x > 0.)
            ...
            float3 raymarch(in float3 from, in float3 dir)
            {
                ...
                float sb = pow(clamp(1. - length(dir.xy) * (sunsize - 0.2) - abs(0.2 - glsl_mod(an, 0.4)), 0., 1.),
                             0.1);
                float sg = pow(
                    clamp(1. - length(dir.xy) * (sunsize - 4.5) - 0.5 * abs(0.2 - glsl_mod(an, 0.4)), 0., 1.), 3.);
                float y = lerp(0.45, 1.2, pow(smoothstep(0., 1., 0.75 - dir.y), 2.)) * (1. - sb * 0.5);
                float3 backg = float3(0.5, 0., 1.) * ((1. - s) * (1. - sg) * y + (1. - sb) * sg * float3(1., 0.8, 0.15)
                    * 3.);
            ...
            float4 frag(v2f __vertex_output) : SV_Target
            {
                ...
                #ifdef BORDER
                color = lerp(((float3)0.), color,
                             pow(max(0., 0.95 - length(oriuv * oriuv * oriuv * float2(1.05, 1.1))), 0.3));
                #endif
